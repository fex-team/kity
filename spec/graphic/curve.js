...
describe("Kity.Curve", function () {
    ...
    it("should extend Parent", function() {
        expect(curve).toImplement(Parent);
    });

    describe("addChild(point [, pos])", function() {
        beforeEach(function() {
            curve.addChild({x: 10, y: 10});
        });
        it("添加点到曲线的关键点集合里", function() {
            expect(curve.getFirstChild()).toMatchPlain({x: 10, y: 10});
        });
    ...
    describe("removeChild(pos)", function() {
        beforeEach(function() {
            curve.addChild({x: 10, y: 10});
            curve.addChild({x: 20, y: 20});
            curve.addChild({x: 30, y: 30});
            curve.removeChild(1);
        });
        it("should remove a key point from the curve in given position", function() {
            expect(curve.getFirstChild()).toMatchPlain({x: 10, y: 10});
            expect(curve.getLastChild()).toMatchPlain({x: 30, y: 30});
        });
    });
